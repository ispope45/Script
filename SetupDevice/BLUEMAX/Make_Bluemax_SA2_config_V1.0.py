...
if __name__ == "__main__":
    ...
    for i in range(2, ws.max_row + 1):
        ...
        if haCls == "HA":
            ...
            ip_skL3 = ws[f'K{row}'].value
            ip_skFw = ws[f'L{row}'].value + "/30"
            ip_ktL3 = ws[f'M{row}'].value + "/28"
            ip_ktFw_vip = ws[f'N{row}'].value + "/28"
            ip_ktFw_rip1 = ws[f'Q{row}'].value + "/29"
            ip_ktFw_rip2 = ws[f'V{row}'].value + "/29"
            ...
            ifCfg_M.append(ssh_make_config('eth12', 1, [ip_ktFw_rip2]))
        ...
        #     with s.put(SUB_URL + EQUIP_API, json=host_data2, headers=headers, verify=False) as res:
        #         write_log(f'{no}_{schName} : {res.url}')
        #
        #     # Routing Setting
        #     ip_ktGw = ip_ktL3.split('/')[0]
