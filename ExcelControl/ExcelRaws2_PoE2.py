...
TARGET_FILE = HOME_PATH + '\\Desktop\\poe3.xlsx'
...
for i in range(firstRow, lastRow+1):
    ...
    if int(ws.Cells(i, Column["COL_H"]).value) == 1:  # 공동회선(병설유치원)
        ...
        # print(string)
        f = open(DST_PATH + str(int(ws.Cells(i, Column["COL_A"]).value)) + "_" + ws.Cells(i, Column["COL_C"]).value + "_BB.txt", "w+")
        f.write(data)
        f.close()
    ...
    for j in range(0, ap_count):
        ...
        script = (
            ...
            f'ip address {ws.Cells(i, Column["COL_S"]).value}{str(ap_ip)}/{str(int(ws.Cells(i, Column["COL_P"]).value))}\n'
            f'exit\n\n'
            f'hostname {ws.Cells(i, Column["COL_H"]).value}_PoE{str(number)}\n'
            f'ip route 0.0.0.0/0 {ws.Cells(i, Column["COL_R"]).value}\n\n'
            f'service ssh\n'
            f'ip ssh port 9992\n\n'
            f'end\n'
        ...
        data = script + EXTRA_COMMENT
        # print(string)
        f = open(DST_PATH + str(int(ws.Cells(i, Column["COL_A"]).value)) + "_" + ws.Cells(i, Column["COL_C"]).value + "_PoE" + str(number) + ".txt", "w+")
        f.write(data)
        f.close()
